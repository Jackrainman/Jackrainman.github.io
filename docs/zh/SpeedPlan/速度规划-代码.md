...
### chassis_speed_planner 模块调用示例（C / FreeRTOS）

以下示例展示如何在 FreeRTOS 控制任务中使用 `chassis_speed_planner` 模块，
完整覆盖 **初始化 → 周期更新 → 运行时切换算法 → 重置** 的典型流程。

```c
#include "chassis_speed_planner.h"

/* 全局（或文件级）规划器实例 */
static SpeedPlanner_t g_planner;

/**
 * @brief 初始化速度规划器，在系统启动时调用一次
 */
void chassis_speed_planner_setup(void)
{
    SpeedPlannerCfg cfg = {
        /* ---- 加速度限制（三种算法通用） ---- */
        .max_accel_x = 3000.0f,     /* mm/s²  X轴最大加速度 */
        .max_accel_y = 3000.0f,     /* mm/s²  Y轴最大加速度 */
        .max_accel_w = 10.0f,       /* rad/s² 偏航轴最大角加速度 */

        /* ---- S曲线专用: jerk 限制 ---- */
        .max_jerk_x  = 20000.0f,    /* mm/s³ */
        .max_jerk_y  = 20000.0f,    /* mm/s³ */
        .max_jerk_w  = 80.0f,       /* rad/s³ */

        /* ---- 防侧翻限速 ---- */
        .enable_rollover      = 1,
        .rollover_threshold   = 200.0f,   /* mm/s  低于此值不限制 */
        .rollover_base_limit  = 300.0f,   /* mm/s  基础限制 */
        .rollover_max_limit   = 800.0f,   /* mm/s  最大限制 */

        /* ---- 系统参数 ---- */
        .tick_freq_hz = 1000,                   /* FreeRTOS tick 频率 */
        .plan_type    = SPEED_PLAN_COSINE,      /* 默认使用余弦平滑 */
    };

    speed_planner_init(&g_planner, &cfg);
}
...
void chassis_control_task(void *param)
{
    ...
    for (;;) {
        /* 1. 获取遥控器/上位机的目标速度 */
        float target_vx = remote_get_vx();   /* mm/s  */
        float target_vy = remote_get_vy();   /* mm/s  */
        float target_vw = remote_get_vw();   /* rad/s */

        /* 2. 速度规划：目标速度 → 平滑输出速度 */
        float out_vx, out_vy, out_vw;
        speed_planner_update(&g_planner,
                             target_vx, target_vy, target_vw,
                             &out_vx, &out_vy, &out_vw);

        /* 3. 将平滑速度送入底盘解算 / 电机控制 */
        chassis_set_speed(out_vx, out_vy, out_vw);
...
/**
 * @brief 运行时切换到 S型曲线算法（例如在某个模式切换回调中调用）
 */
void switch_to_scurve(void)
{
    speed_planner_set_type(&g_planner, SPEED_PLAN_SCURVE);
}

/**
 * @brief 紧急停止 / 模式切换时重置规划器
 *        速度归零，配置保留，下次 update 重新从零开始规划
 */
void chassis_emergency_stop(void)
{
    speed_planner_reset(&g_planner);
}
```
...
| 步骤 | 关键点 |
|------|--------|
| 初始化 | `speed_planner_init` 会将实例清零并拷贝配置，`tick_freq_hz` 为 0 时自动取 1000 |
| 周期更新 | `speed_planner_update` 内部通过 `xTaskGetTickCount()` 自动计算 dt，首次调用只记录时间戳并输出零速 |
| 算法切换 | `speed_planner_set_type` 可在运行中热切换，切换后下一次 update 即生效 |
| 重置 | `speed_planner_reset` 保留配置，仅清零速度/加速度/余弦状态 |
| 防侧翻 | 当 `enable_rollover = 1` 时自动在 update 末尾执行，限制 XY 平面速度向量的周期变化量 |
